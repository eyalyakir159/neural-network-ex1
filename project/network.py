...
class newwork:
    ...
    def SGD(self,data_reader:api.data_reader,learning_rate,ephocs,branch_size):
        """
        ...
        for j in range(ephocs):
            ...
            for mini_batch in data_reader.batches:
                mean_grad_b,mean_grad_W = self.update_mini_branch(mini_batch, learning_rate)
                for data in mini_batch:
                    log_loss+=self.loss_log(self.feed_fowords(data.data_to_vector()),data.target_to_vector())
                gradient = []
        ...
        self.save("test")
    def update_mini_branch(self,branch,learning_rate):
